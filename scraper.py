...
def scrape_match_for_summoner(riot_name, tag, server):
    ...
    try:
        ...
        for participant in participants:
            ...
            print(f"  → ChampionID: {champ_id} ({champ_name})")

            try:
                rank, general_winrate = get_rank_info(summoner_id, region)
                print(f"  ✅ Rank fetched: {rank}, WR: {general_winrate}%")
            except Exception as e:
                print(f"  ❌ Failed to fetch rank info: {e}")
                rank, general_winrate = "Unknown", 50.0

            try:
                stats = asyncio.run(get_recent_match_stats_async(player_puuid, match_region, champ_id, match_count=10))
                print(f"  ✅ Stats fetched:")
                print(f"     KDA: {stats['kda']}")
                print(f"     GPM: {stats['gold_per_minute']}")
                print(f"     DPM: {stats['damage_per_minute']}")
                print(f"     Champ WR: {stats['champion_winrate']} ({stats['champ_games']} games)")
            except Exception as e:
                print(f"  ❌ Failed to fetch recent match stats: {e}")
                stats = {
                    "kda": 2.5,
                    "gold_per_minute": 400,
